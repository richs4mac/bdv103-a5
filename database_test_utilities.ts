...
import { type Book, type BookID, type OrderId, type ShelfId } from './adapter/assignment-4'
import { type BookDatabaseAccessor } from './src/books/books_database'
import { type WarehouseDatabaseAccessor } from './src/warehouse/types'
...
export async function seedWarehouseDatabase (accessor: WarehouseDatabaseAccessor,
  { books, orders }: { books: Record<BookID, Record<ShelfId, number>>, orders: Record<OrderId, Record<BookID, number>> }):
  Promise<void> {
  await Promise.all([
